...
export async function extractPeople(text, deps = {}) {
  ...
  // One retry, and only if it fits inside the same 8 seconds this always had.
  // The answer has already gone out; this is the lowest-priority call Oscar
  // makes, and it must never be the reason the account is over its limit when
  // the next real question arrives. Failures still throw — catchPeople below is
  // where rule 4 is enforced, and it wants to know what went wrong.
  const { res, rawText } = await postWithRetry(
    ...
    { budgetMs: CATCH_TIMEOUT_MS, attempts: 2, sleep: deps.sleep, random: deps.random }
  );
