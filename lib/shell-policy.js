...
export const DENY_PATTERNS = [
  ...
  // `> /dev/sda` does what `dd of=/dev/sda` does. /dev/null and the standard
  // streams are exempt: redirecting into them is routine and harmless.
  { re: />\s*\/dev\/(?!null|stdout|stderr|tty|fd\/)/i, why: 'writing straight to a device' },
 ...
 */
/**
 * The same split, but keeping what separated each piece from the last.
 *
 * The distinction that matters is redirects. In `echo hi > notes.txt` the
 * shell runs ONE program and opens a file; `notes.txt` is a filename and is
 * never executed. Checking it against the allowlist — which is what happened
 * before this existed — refuses the single most ordinary way to write a file,
 * with the baffling complaint that "notes.txt" is not an allowed program.
 *
 * Redirect targets are still scanned by both pattern tiers along with the rest
 * of the line. They are exempt from the allowlist only, because the allowlist
 * answers "may this program run" and a redirect target is not a program.
 *
 * @returns {{text: string, isRedirectTarget: boolean}[]}
 */
export function splitCommand(command) {
  ...
  const push = (isRedirectTarget) => {
    out.push({ text: buf.trim(), isRedirectTarget });
    buf = '';
  };
...
/**
 * Does this fragment make the shell run something to work out what it says?
 *
 * `> out.txt` is a filename and needs no allowlist check. `> $(whoami).txt`
 * and its backtick spelling are a command wearing a filename, and do need
 * one. A bare `$HOME` is only expansion, not execution, so it stays exempt —
 * checking it would refuse `> "$HOME/out.txt"` for no benefit.
 */
const RUNS_SOMETHING = /\$\(|`/;

/** Just the text of each segment. The shape most callers and tests want. */
export function splitSegments(command) {
  return splitCommand(command).map((part) => part.text);
}
...
export function classifyCommand(command, options = {}) {
  ...
  for (const part of parts) {
    for (const { re, why } of DENY_PATTERNS) {
      if (re.test(part.text)) return { verdict: 'refuse', reason: `Refused: ${why}.` };
    }
    ...
    if (part.isRedirectTarget && !RUNS_SOMETHING.test(part.text)) continue;
    const program = programOf(part.text);
  ...
  if (options.mode !== 'unrestricted') {
    ...
    for (const part of parts) {
      // A redirect target is a filename. The allowlist answers "may this
      // program run", which is not a question about a filename — unless the
      // filename is computed by running something, in which case it is.
      if (part.isRedirectTarget && !RUNS_SOMETHING.test(part.text)) continue;
      const program = programOf(part.text);
      if (!program) continue;
