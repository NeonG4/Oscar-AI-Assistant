 ...
 * WHY THIS EXISTS
 *
 * `isGoogleConfigured` and `canWriteGoogle` only look at environment
 * variables, which means /api/health can report `writesEnabled: true` while
 * every write actually fails with 403. Two causes, both invisible:
 *
 *   1. The refresh token was minted before OSCAR_ALLOW_WRITES was set. Scopes
 *      are fixed at authorisation time by scopesFor(), so a token granted back
 *      then carries the five read-only scopes forever. Flipping the env var
 *      afterwards changes nothing — you have to authorise again.
 *
 *   2. The API is switched off in Cloud Console. Enabling Calendar does not
 *      enable Tasks or Docs, and those two are the ones people miss.
 *
 * So this asks Google both questions directly: what scopes does the stored
 * grant actually hold, and does each API answer a real request.
 *
 * KEEP THIS OUTPUT NON-IDENTIFYING. /api/health is public on purpose, so the
 * probe reports scope names and statuses only — never an address, a file name,
 * a subject line, or a count of anything.
 */
...
/** Which scope grants read, and which grants write, per service. */
const SERVICE_SCOPES = {
  calendar: { read: ['calendar.readonly', 'calendar'], write: ['calendar'] },
  tasks: { read: ['tasks.readonly', 'tasks'], write: ['tasks'] },
  // Two write scopes, and they are not interchangeable: modify covers drafting
  // and trashing, send covers sending. Reported separately so a token missing
  // only gmail.send says exactly that rather than "gmail: read".
  gmail: { read: ['gmail.readonly', 'gmail.modify'], write: ['gmail.modify', 'gmail.send'] },
  drive: { read: ['drive.readonly', 'drive'], write: ['drive'] },
  docs: { read: ['documents.readonly', 'documents'], write: ['documents'] },
};

/**
 * One cheap read per API. The point is not the data — it is the status code.
 *
 * Docs is the odd one out: every read needs a document id, so there is nothing
 * to list. Asking for an id that cannot exist is the trick — 404 means the API
 * is on and the grant is good, which is exactly what we wanted to know, while
 * 403 means disabled or unscoped. Nothing is created either way.
 */
const PROBE_URLS = {
  calendar: () =>
    'https://www.googleapis.com/calendar/v3/calendars/primary/events?maxResults=1&timeMin=' +
  ...
  docs: () => 'https://docs.googleapis.com/v1/documents/oscarHealthProbeNoSuchDocument',
};

/**
 * Google's "API not enabled" message names your Cloud project by number, and
 * this endpoint is public. Strip anything that long and numeric.
 */
function scrub(message) {
  return String(message || '')
    .replace(/\d{6,}/g, '<project>')
    .slice(0, 200);
}

function classifyScopes(granted) {
  const out = {};
  for (const [service, wanted] of Object.entries(SERVICE_SCOPES)) {
    const has = (short) => granted.has(SCOPE_PREFIX + short);
    const missingWrite = wanted.write.filter((s) => !has(s));
    out[service] = {
      read: wanted.read.some(has),
      write: missingWrite.length === 0,
      ...(missingWrite.length ? { missing: missingWrite } : {}),
    };
  }
  return out;
}
...
/**
 * Ask Google what the stored grant can really do.
 *
 * Never throws: a probe that blows up is itself the answer, so failures come
 * back as fields to read rather than an exception that takes /api/health down
 * with it.
 *
 * @returns {Promise<object>} safe to serialise into a public response
 */
export async function probeGoogle(deps = {}) {
  const env = deps.env || process.env;
  const doFetch = deps.fetchImpl || globalThis.fetch;

  if (!isGoogleConfigured(env)) {
    return { ran: false, reason: 'Google is not connected — no client id, secret or refresh token.' };
  }

  // Everything below needs a token, so a failure here ends the probe. This is
  // also where an expired refresh token surfaces, with the 7-day Testing-status
  // explanation already attached by getAccessToken.
  let accessToken;
  try {
    accessToken = await getAccessToken(deps);
  } catch (err) {
    return { ran: true, token: 'failed', error: scrub(err && err.message), scopes: null, apis: null };
  }

  // ---- what the grant holds ----------------------------------------------
  let scopes = null;
  let scopeError;
  try {
    const res = await doFetch(`${TOKENINFO_URL}?access_token=${encodeURIComponent(accessToken)}`);
    const info = await res.json();
    const granted = new Set(String((info && info.scope) || '').split(' ').filter(Boolean));
    scopes = granted.size ? classifyScopes(granted) : null;
    if (!scopes) scopeError = 'Google returned no scope list for this token.';
  } catch (err) {
    scopeError = scrub((err && err.message) || 'Could not read the token scopes.');
  }

  // ---- whether each API answers ------------------------------------------
  // In parallel: five sequential round trips to Google is slow enough to risk
  // the function timeout, and they do not depend on each other.
  const names = Object.keys(PROBE_URLS);
  const results = await Promise.all(
    names.map(async (name) => {
      try {
        await googleFetch(PROBE_URLS[name](), {}, { ...deps, env });
        return [name, { ok: true }];
      } catch (err) {
        const status = err && err.status;
        // The deliberate miss. A 404 from Docs proves the API is live and the
        // token is accepted, which is a pass, not a failure.
        if (name === 'docs' && (status === 404 || status === 400)) return [name, { ok: true }];
        return [name, { ok: false, status: status || null, error: scrub(err && err.message) }];
      }
    })
  );

  const apis = Object.fromEntries(results);

  // ---- the summary a person actually reads --------------------------------
  const writesOn = canWriteGoogle(env);
  const problems = [];

  for (const name of names) {
    if (!apis[name].ok) {
      problems.push(
        apis[name].status === 403
          ? `${name}: denied — enable the API in Cloud Console, or the grant is missing its scope`
          : `${name}: read failed (${apis[name].status || 'no status'})`
      );
    }
  }
  ...
  if (scopes && writesOn) {
    for (const name of names) {
      if (!scopes[name].write) {
        problems.push(
          `${name}: no write scope on the stored token — re-run \`npm run google-auth\` with ` +
            'OSCAR_ALLOW_WRITES=1 set, then update GOOGLE_REFRESH_TOKEN'
        );
      }
    }
  }
  ...
  return {
    ran: true,
    token: 'ok',
    readable: names.every((n) => apis[n].ok),
    writable: Boolean(scopes) && writesOn && names.every((n) => scopes[n].write),
    scopes,
