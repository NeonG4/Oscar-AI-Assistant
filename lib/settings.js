 ...
 *
 * HOW MUCH OSCAR ASKS, ACROSS EVERY KIND OF ACTION
 *
 ...
 *
 * WHETHER OSCAR MAY USE YOUR COMPUTER AT ALL
 *
 *   off      nothing runs. The tool is withheld and the runner stops claiming.
 *   confirm  every command asks first. (default)
 *   open     commands run without asking; the denylist still applies.
 *
 * Separate from the confirm level above because the first thing you want from
 * a capability that executes arbitrary code is an off switch, and no amount of
 * "how much does it ask" expresses that. See COMMAND_POLICIES at the bottom.
 */
 ...
 *
 * Deliberately its own setting rather than a fourth value on the confirm level
 * above. That one is about the WEIGHT of an action — is deleting an email worse
 * than reading one — and applies to email, calendar and files alike. This one is
 * about a single capability that happens to be the only one that can execute
 * arbitrary code on hardware you own, and the first thing you want from it is a
 * switch that turns it off entirely. No level of "how much does it ask" says
 * that.
 *
 *   off      Oscar cannot run anything. run_cmd is withheld from the model, and
 *            the runner is told to stop claiming work. Both ends, on purpose.
 *   confirm  Every command asks first, on a device you are holding. (default)
 *   open     Commands run without asking. The denylist still applies — there is
 *            no setting that reformats a disk.
 *
 * DEFAULT IS `confirm`, NOT `open`. A deployment where nobody has touched the
 * dropdown should be one where nothing executes unattended, and every failure
 * path here lands on the stricter answer for the same reason.
 */
export const COMMAND_POLICIES = ['off', 'confirm', 'open'];

/** Ask first. The answer a deployment that has never been configured should get. */
export const DEFAULT_COMMAND_POLICY = 'confirm';
...
/**
 * @returns {'off'|'confirm'|'open'} the policy, or the default for anything else.
 *
 * Note which way an unrecognised value falls: onto `confirm`, never `open`. A
 * typo in the database must not be the thing that turns the asking off.
 */
 ...
 * request. An unreadable answer means `confirm`, which is the direction that
 * asks rather than the one that runs.
 *
 * @returns {Promise<'off'|'confirm'|'open'>}
 */
