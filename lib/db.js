...
/**
 * The same request helper, exposed for other modules that talk to Supabase
 * (lib/plans.js). Resolves the config itself so callers don't repeat it.
 *
 * Unlike the logging functions above, this one is used for data the user
 * actually asked for — so it reports failures rather than swallowing them, and
 * callers are expected to surface the message.
 *
 * @param {string} path  everything after /rest/v1/, e.g. "plans?id=eq.3"
 * @returns {Promise<{ok: boolean, status?: number, data?: any, error?: string}>}
 */
export async function dbRequest(path, init = {}, deps = {}) {
  const env = deps.env || process.env;
  const config = dbConfig(env);
  if (!config) return { ok: false, error: 'No database is configured.' };
  const doFetch = deps.fetchImpl || globalThis.fetch;
  return request(path, init, config, doFetch);
}
 ...
 *
 * Newest first, EXCEPT when reading a single thread: a conversation is read the
 * way it was had, oldest turn at the top, so the caller never has to reverse it
 * before showing it or feeding it back to the model.
 *
 * @param {{limit?: number, search?: string, before?: string, conversation?: string}} [opts]
...
export async function recentConversations(opts = {}, deps = {}) {
  ...
  const thread = opts.conversation ? String(opts.conversation) : '';
  const limit = Math.min(Math.max(Number(opts.limit) || 25, 1), 100);
  const params = new URLSearchParams({
    select:
      'id,created_at,conversation_id,question,answer,detail,title,ok,error,model,via,source,total_ms,total_tokens,tools_used',
    order: thread ? 'created_at.asc' : 'created_at.desc',
    limit: String(limit),
  });

  // A uuid and nothing else. PostgREST would reject anything odd anyway, but a
  // filter value assembled from user input is worth pinning down at the source.
  if (thread) {
    if (!/^[0-9a-fA-F-]{16,40}$/.test(thread)) return { ok: true, rows: [] };
    params.set('conversation_id', `eq.${thread}`);
  }
...
/**
 * One thread, as messages the model can read.
 *
 * This is what makes a follow-up question work: "and what about tomorrow?" only
 * means anything if the turn before it is in front of the model. Each stored row
 * is one exchange, so it expands into two messages.
 *
 * Failures come back as an empty list rather than an error. A conversation that
 * has lost its memory still answers the question in front of it, which is much
 * better than refusing to answer at all because a SELECT timed out.
 *
 * @param {string} conversationId
 * @returns {Promise<Array<{role: 'user'|'assistant', content: string}>>}
 */
export async function conversationTurns(conversationId, opts = {}, deps = {}) {
  if (!conversationId) return [];

  const result = await recentConversations(
    { conversation: conversationId, limit: opts.limit || 20 },
    deps
  );
  if (!result.ok) return [];
  ...
  for (const row of result.rows) {
    if (row.question) messages.push({ role: 'user', content: row.question });
    // A failed turn contributes nothing useful to say back, but the question is
    // still context — so the answer is skipped and the question is not.
    if (row.ok && row.answer) messages.push({ role: 'assistant', content: row.answer });
  }
  return messages;
}
...
export function conversationRow({
  ...
  return {
    ...
    // Which back-and-forth this turn belongs to. Null for anything asked
    // outside a thread — a one-shot Shortcut question, or a row written before
    // conversations existed — and the history view treats those as single turns.
    ...
    // Names only. Tool RESULTS can contain your coordinates, so they are
    // deliberately not logged.
    tools_used: result && result.toolsUsed && result.toolsUsed.length ? result.toolsUsed : null,
