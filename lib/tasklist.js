 ...
 * lib/tasklist.js
 * ----------------------------------------------------------------------------
 * The visible plan of a single run: what Oscar decided to do, and how far
 * through it he is.
 *
 * WHY THIS EXISTS SEPARATELY FROM lib/plans.js
 *
 * A plan (lib/plans.js) is a durable thing you keep — it lives in the database,
 * survives the run that made it, and you can tick steps off it next week. A
 * task list is the opposite: it belongs to one run, exists so the person
 * watching can see what is happening, and is thrown away with the job.
 *
...
/** Beyond this a "task list" has stopped being a plan and become a monologue. */
