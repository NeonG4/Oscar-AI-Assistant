/**
 * lib/jobs.js
 * ----------------------------------------------------------------------------
 * Asynchronous agent runs.
 *
 * THE PROBLEM THIS SOLVES
 *
 * A synchronous request is bounded by whoever is waiting on it: an iOS Shortcut
 * gives up after a few seconds, and a Vercel function dies at maxDuration
 * regardless. `waitUntil` does not help — Vercel's docs are explicit that
 * promises passed to it share the function's own timeout.
 *
 * But the budget resets per INVOCATION. So a long run is spread across several:
 * each one loads the state, does what it can, writes the state back, and hands
 * off. The job row is the baton.
 *
 * TWO THINGS CAN CARRY A JOB FORWARD
 *
 *   1. The web app, polling and calling /api/step — gives live progress.
 *   2. The function itself, POSTing to /api/step before it runs out.
 *
 * Both is best: the app shows progress when open, and work continues when it
 * isn't. Either is enough on its own, which is the point — no single component
 * has to stay alive.
 */

import { dbRequest, isConfigured } from './db.js';
import { signToken, verifyToken, sessionSecret } from './auth.js';

export { isConfigured as isJobsConfigured };

/** How long a job may run in total before it is abandoned. */
export const JOB_TTL_MS = 15 * 60 * 1000;

/** Hard ceiling on steps per job, independent of the agent's own budgets. */
export const MAX_JOB_STEPS = 40;

/**
 * Statuses that mean the job has not finished with you yet.
 *
 * The web app keeps its own copy of this list for rendering. This one is the
 * authority for decisions the server makes — currently one: whether removing a
 * job is pulling the rug out from under a run that is still going.
 */
export const ACTIVE_STATUSES = ['queued', 'running', 'awaiting_confirm', 'awaiting_answer'];

export class JobError extends Error {
  constructor(message, status = 500) {
    super(message);
    this.name = 'JobError';
    this.status = status;
  }
}

/* ----------------------------------------------------------- step authority */

/**
 * A short-lived token proving "you may advance THIS job".
 *
 * /api/step must not be drivable by strangers — it spends your OpenAI credit.
 * Rather than putting a long-lived shared secret on an internal call, each hop
 * mints a fresh token for the next one, scoped to a single job id.
 */
export function createJobToken(jobId, env = process.env, now = Date.now()) {
  return signToken({ t: 'job', id: String(jobId), exp: now + JOB_TTL_MS }, sessionSecret(env));
}

/** @returns {string|null} the job id the token authorises, or null. */
export function readJobToken(token, env = process.env) {
  const payload = verifyToken(token, sessionSecret(env));
  return payload && payload.t === 'job' ? payload.id : null;
}

/* --------------------------------------------------------------------- CRUD */

function tidyJob(row) {
  if (!row) return null;
  return {
    id: row.id,
    status: row.status,
    question: row.question,
    mode: row.mode || undefined,
    model: row.model || undefined,
    steps: row.steps,
    events: Array.isArray(row.events) ? row.events : [],
    // The task list of the run: what Oscar decided to do and how far through it
    // he is. See lib/tasklist.js — this is what the web app renders when the
    // thinking panel is in its plain-language mode.
    tasks: Array.isArray(row.tasks) ? row.tasks : [],
    conversationId: row.conversation_id || undefined,
    title: row.title || undefined,
    answer: row.answer || undefined,
    detail: row.detail || undefined,
    error: row.error || undefined,
    pendingConfirmation: row.pending_confirm || undefined,
    questionId: row.question_id || undefined,
    totalTokens: row.total_tokens || undefined,
    createdAt: row.created_at,
    updatedAt: row.updated_at,
    finishedAt: row.finished_at || undefined,
  };
}

export async function createJob(input, deps = {}) {
  if (!isConfigured(deps.env || process.env)) {
    throw new JobError('No database is configured, so background work is unavailable.', 503);
  }

  const row = {
    question: String(input.question || '').slice(0, 4000),
    status: 'queued',
    mode: input.mode || null,
    model: input.model || null,
    state: input.state || null,
    source: input.source || null,
    via: input.via || null,
    conversation_id: input.conversationId || null,
  };

  const result = await dbRequest(
    'jobs',
    { method: 'POST', headers: { prefer: 'return=representation' }, body: JSON.stringify(row) },
    deps
  );
  if (!result.ok) throw new JobError(`Could not queue the job: ${result.error || result.status}`);

  const job = Array.isArray(result.data) ? result.data[0] : result.data;
  if (!job || !job.id) throw new JobError('The database did not return a job id.');
  return job;
}

/** @returns the raw row, including `state` — callers that expose it must tidy first. */
export async function loadJob(id, deps = {}) {
  const result = await dbRequest(
    `jobs?id=eq.${encodeURIComponent(id)}&select=*&limit=1`,
    { method: 'GET' },
    deps
  );
  if (!result.ok) throw new JobError(`Could not load the job: ${result.error || result.status}`);
  const job = Array.isArray(result.data) ? result.data[0] : result.data;
  if (!job) throw new JobError('No job with that id.', 404);
  return job;
}

export async function getJob(id, deps = {}) {
  return tidyJob(await loadJob(id, deps));
}

export async function updateJob(id, patch, deps = {}) {
  const body = { ...patch, updated_at: new Date().toISOString() };
  const result = await dbRequest(
    `jobs?id=eq.${encodeURIComponent(id)}`,
    { method: 'PATCH', headers: { prefer: 'return=minimal' }, body: JSON.stringify(body) },
    deps
  );
  if (!result.ok) throw new JobError(`Could not update the job: ${result.error || result.status}`);
  return true;
}

export async function listJobs(opts = {}, deps = {}) {
  const params = new URLSearchParams({
    select:
      'id,created_at,updated_at,finished_at,status,question,mode,model,steps,title,answer,error,tasks,conversation_id',
    order: 'created_at.desc',
    limit: String(Math.min(Math.max(Number(opts.limit) || 20, 1), 50)),
  });
  if (opts.status && opts.status !== 'all') params.set('status', `eq.${opts.status}`);

  const result = await dbRequest(`jobs?${params}`, { method: 'GET' }, deps);
  if (!result.ok) throw new JobError(`Could not list jobs: ${result.error || result.status}`);
  return (Array.isArray(result.data) ? result.data : []).map(tidyJob);
}

/**
 * Remove a job for good.
 *
 * The row is the whole record of a background run, so this is not undoable —
 * which is why a job that is still working refuses unless `force` says the
 * caller meant it. Deleting a live one pulls the state out from under an
 * invocation that is mid-flight; api/step.js copes with that by stopping
 * quietly, but it should still take a second, deliberate decision.
 *
 * What is NOT removed: the answer, if the job produced one. That lives in
 * `conversations` and stays in History, because a job is the machinery that
 * produced an exchange rather than the exchange itself. A mission's stored task
 * list stays too — it is a row in `plans`, and this only ever touches `jobs`.
 *
 * @param {{force?: boolean}} [opts]
 * @returns {Promise<{removed: true, was: string}>} the status it had when it went.
 */
export async function deleteJob(id, opts = {}, deps = {}) {
  const job = await loadJob(id, deps);

  if (ACTIVE_STATUSES.includes(job.status) && !opts.force) {
    throw new JobError('That job is still working. Remove it again to confirm you mean it.', 409);
  }

  const result = await dbRequest(
    `jobs?id=eq.${encodeURIComponent(id)}`,
    { method: 'DELETE', headers: { prefer: 'return=minimal' } },
    deps
  );
  if (!result.ok) throw new JobError(`Could not remove the job: ${result.error || result.status}`);

  return { removed: true, was: job.status };
}

/* ------------------------------------------------------------- transitions */

export async function markRunning(id, deps = {}) {
  return updateJob(id, { status: 'running' }, deps);
}

/**
 * Checkpoint mid-run.
 *
 * `tasks` is written on its own column rather than left inside `state`, because
 * the two have different lifetimes: state is deliberately thrown away when a job
 * finishes (it is the bulk of the row), while the task list is the record of
 * what was done and is worth keeping afterwards.
 */
export async function saveProgress(id, { state, events, steps, tasks }, deps = {}) {
  return updateJob(
    id,
    { status: 'running', state, events, steps, ...(tasks ? { tasks } : {}) },
    deps
  );
}

export async function markDone(id, result, deps = {}) {
  return updateJob(
    id,
    {
      // A run that answered without finishing its task list is not 'done'.
      // Recording "finished" against a job with four tasks still open is the
      // single most misleading thing this table could say, so it gets a status
      // of its own and every reader can tell the two apart.
      status: result.incomplete ? 'incomplete' : 'done',
      title: result.title || null,
      answer: result.answer || null,
      detail: result.detail || null,
      events: result.events || [],
      tasks: result.tasks || [],
      steps: result.rounds || 0,
      total_tokens: (result.usage && result.usage.total_tokens) || null,
      model: result.model || null,
      // The message history is the bulk of the row and is useless once the job
      // is finished. Dropping it here keeps the table small.
      state: null,
      pending_confirm: null,
      finished_at: new Date().toISOString(),
    },
    deps
  );
}

export async function markAwaitingConfirm(id, { state, result }, deps = {}) {
  return updateJob(
    id,
    {
      status: 'awaiting_confirm',
      // State is KEPT here, unlike markDone — the run resumes after the answer.
      state,
      events: result.events || [],
      tasks: result.tasks || [],
      steps: result.rounds || 0,
      title: result.title || null,
      answer: result.answer || null,
      pending_confirm: result.pendingConfirmation || null,
    },
    deps
  );
}

/**
 * Park a run that stopped to ask you something.
 *
 * State is KEPT, like markAwaitingConfirm — the run resumes into the middle of
 * the round it froze. The difference is how long this may last: a confirmation
 * is answered in seconds while you are looking at the screen, and a question
 * might sit until tomorrow morning. Nothing here times out on its own.
 */
export async function markAwaitingAnswer(id, { state, result, questionId }, deps = {}) {
  return updateJob(
    id,
    {
      status: 'awaiting_answer',
      state,
      events: (result && result.events) || [],
      tasks: (result && result.tasks) || [],
      steps: (result && result.rounds) || 0,
      title: (result && result.title) || 'A question',
      answer: (result && result.answer) || null,
      question_id: questionId || null,
    },
    deps
  );
}

export async function markFailed(id, message, deps = {}) {
  return updateJob(
    id,
    {
      status: 'failed',
      error: String(message || 'Something went wrong.').slice(0, 1000),
      state: null,
      finished_at: new Date().toISOString(),
    },
    deps
  );
}

/**
 * Where to POST to continue a job.
 *
 * VERCEL_URL is the deployment's own hostname, which is what you want for an
 * internal hop — it reaches this exact deployment rather than whatever the
 * production alias currently points at. Returns null when running locally,
 * where self-continuation is skipped and the client drives instead.
 */
export function selfUrl(env = process.env) {
  if (env.OSCAR_BASE_URL) return env.OSCAR_BASE_URL.replace(/\/+$/, '');
  if (env.VERCEL_URL) return `https://${env.VERCEL_URL}`;
  return null;
}

/**
 * How long to hold the current invocation open after firing the next one.
 *
 * NOT a wait for the work — the whole design exists to avoid that. It is a wait
 * for the REQUEST to leave the machine. A serverless function stops existing
 * the moment it responds, which is the same reason push notifications are
 * awaited in api/step.js: a hop that has only been queued on the event loop is
 * a hop that never happens, and a job whose baton was dropped at that moment
 * has nothing left to restart it.
 */
export const HANDOFF_FLUSH_MS = 800;

function flushDelay(ms) {
  return new Promise((resolve) => {
    const timer = setTimeout(resolve, ms);
    // Never hold a local `node server.js` open waiting on this.
    if (timer && typeof timer.unref === 'function') timer.unref();
  });
}

/**
 * Kick off the next invocation and DO NOT wait for it to finish.
 *
 * The response is deliberately never awaited — that would recreate the problem
 * this whole design exists to avoid. What IS awaited is the brief moment it
 * takes the request to be dispatched; see HANDOFF_FLUSH_MS for why that
 * difference matters. Failures are logged, not thrown: if the hop is lost, the
 * web app polling picks the job up instead, which is exactly why there are two
 * ways to advance a job.
 *
 * @returns {Promise<boolean>} whether a hop was fired at all.
 */
export async function continueJob(jobId, deps = {}) {
  const env = deps.env || process.env;
  const base = selfUrl(env);
  if (!base) return false;

  const doFetch = deps.fetchImpl || globalThis.fetch;
  const flushMs = Number(deps.flushMs ?? HANDOFF_FLUSH_MS);

  try {
    const request = Promise.resolve(
      doFetch(`${base}/api/step`, {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ jobId, token: createJobToken(jobId, env) }),
      })
    ).catch((err) => console.error(`[oscar] could not continue job ${jobId}: ${err && err.message}`));

    // Whichever comes first: the request is on its way, or we have given it
    // long enough that it must be.
    await Promise.race([request, flushDelay(flushMs)]);
    return true;
  } catch (err) {
    console.error(`[oscar] could not continue job ${jobId}: ${err && err.message}`);
    return false;
  }
}
