 ...
 *
 * So a mission does NOT keep one long conversation. It keeps a PLAN, and runs a
 * SEPARATE, SHORT conversation for each step of it.
 *
 *   the plan   — the durable memory. Which steps exist, which are done.
 ...
 * THREE PHASES
 *
 *   planning  — one agent run whose only job is to produce a plan
...
export function createMissionState(input, env = process.env) {
  ...
  return {
    ...
    agent: null,
    events: [],
    // Mirrors the plan's steps, so the web app can render a mission's progress
    // with exactly the same task list it renders for an ordinary run. Filled in
    // once the plan exists; see the working phase below.
    tasks: [],
...
function planningPrompt(goal) {
  return [
    `Your goal: ${goal}`,
    '',
    'Do not attempt the goal yet. Your only job right now is to break it into steps.',
    '',
    ...
    '',
    'Write steps for YOURSELF to do, not instructions for the user. If the goal',
    'needs files written or commands run, say so in the step. Do not pad the',
    'plan with steps like "review the results" or "consider next steps".',
    '',
    'After create_plan returns, reply with one short sentence naming the plan.',
  ].join('\n');
}
...
function taskPrompt({ goal, plan, step, notes }) {
  ...
  const lines = [
    ...
    '',
    `The plan is "${plan.title}" and has ${plan.steps.length} steps.`,
  ];
  ...
  lines.push(
    ...
    '',
    'Do that one step and nothing further down the plan. Use your tools to',
    ...
    'Do not call complete_plan_step, create_plan or plan_tasks — the plan you are',
    'working through IS the task list, and the mission ticks it off for you.'
  );
...
export async function runMissionStep(state, deps = {}) {
  ...
  if (next.phase === 'working') {
    ...
    // The plan IS the task list here, so it is copied across on every pass
    // rather than tracked separately — the database row is the truth about
  ...
  // The plan is finished, so it should stop showing up as active work.
  if (merged.planId) {
