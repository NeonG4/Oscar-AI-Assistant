...
import { createAgentState, runAgentStep, resumeWithAnswer, isAwaitingAnswer } from './agent.js';
...
function taskPrompt({ goal, plan, step, notes }) {
  ...
  lines.push(
    ...
    'Do not call complete_plan_step or create_plan — the mission handles that.'
  );
 ...
 * call it never made.
 */
function subAgent(state, question, env, { tools = true } = {}) {
...
export async function runMissionStep(state, deps = {}) {
  ...
  if (next.phase === 'planning') {
    if (!next.agent) next.agent = subAgent(next, planningPrompt(next.goal), env);
