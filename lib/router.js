 ...
 */
const DEEP_MARKERS =
  /\b(plan|organi[sz]e|schedule out|strategy|roadmap|itinerary|curriculum|workout|routine|budget|research|compare .* and .* and|(write|create|make|draft) (me )?(a|an|some)( \w+){0,3} (story|essay|article|poem|report|letter|write-?up|summary)|draft (a|an)|step by step|walk me through|help me (build|set up|prepare|figure out))\b/i;
...
/**
 * Things Oscar puts WORDS into.
 *
 * A document, an email or a note is where prose gets delivered — not evidence
 * that something is being built. "Write me a story and store it in a Google
 * Doc" is one piece of writing and one tool call; run as a mission it becomes a
 * task list, and a task list that says "create a document" one step after
 * "write the story" is how a user ends up with three documents instead of one.
 *
 * Checked before the classifier is spent, because the classifier reads "store
 * it in a Doc" as something being built often enough to matter.
 */
const PROSE_TARGETS =
  /\b(story|essay|article|poem|letter|write-?up|blog post|speech|recipe|google doc|document|doc)\b/i;
...
export function quickClassify(question) {
  ...
  // Asking for words to be WRITTEN — and it takes both halves, so that "what
  // is in my doc" is still the lookup it looks like. Last, so it only catches
  // what would otherwise have gone to the classifier, which reads "put it in a
  // Google Doc" as software far too readily.
  if (/\b(write|compose|draft|create|make|put|save|store)\b/i.test(text) && PROSE_TARGETS.test(text)) {
    return 'deep';
  }
...
const SYSTEM = [
  ...
  'read is deep, working software is mission. WHERE those words are put makes no',
  'difference — a story, a letter or a write-up is deep whether it is spoken,',
  'emailed, or saved into a Google Doc. When in doubt choose deep — it is much',
  'cheaper to run.',
  '',
